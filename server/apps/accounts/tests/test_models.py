...
class TestUser(TestCase):
    ...
    def test_create_user_with_existing_username(self):
        ...
        with self.assertRaises(IntegrityError):
            User.objects.create_user(
                email="existing_user2@test.com", username="existing_user")

    def test_create_user_with_existing_username_method(self):
        """We are checking, if we can create a user with just an email."""
        new_user = User.objects.create_user(
            email="new_user@test.com", password="test")
        self.assertEqual(User.objects.get(email="new_user@test.com"), new_user)
    ...
    def test_find_next_available_username(self):
        ...
        self.assertEqual(User.objects.find_next_available_username(
        ...
        while i < 12:
            self.assertEqual(User.objects.find_next_available_username(
                "existing_user"), "existing_user{}".format(i))
            User.objects.create_user(
                email="existing_user@test{}.com".format(i), password="test")
            i += 1
