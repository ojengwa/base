...
class AccountAdapter(DefaultAccountAdapter):
    ...
    def generate_unique_username(self, txts, regex=None):
        """Use a given username and find the next free ID
         - if not use the first part of an email."""

        username = txts[3]

        if username is None or username == '':
            email = txts[2]
            return User.objects.find_next_available_username(
                email.split("@")[0])
        else:
    ...
    def send_mail(self, template_prefix, email, context):
        ...
        create_and_send_mail(template_name=template_prefix,
                             context=context_dict, to_address=email)

    def get_email_confirmation_redirect_url(self, request):
        """The URL to return to after successful e-mail confirmation."""

        # TODO: Maybe have a JSON response, when content-type=JSON? Needs to be
