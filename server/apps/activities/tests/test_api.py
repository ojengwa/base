...
from server.apps.comments.tests.factories import CommentUserFactory
...
class TestGetActivities(APITestCase):
    ...
    def test_result_should_contain_right_actor(self):
        assert self.response.data['results'][0][
            'actor']['id'] == str(self.comment.author.pk)
    ...
    def test_result_should_contain_action_object_with_right_content(self):
        assert self.response.data['results'][0][
            'action_object']['content'] == self.comment.content

    def test_result_should_contain_right_target(self):
        assert self.response.data['results'][0][
            'target']['id'] == str(self.commented_user.pk)
