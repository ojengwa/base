...
from server.apps.users.tests.factories import UserFactory, VerifiedUserFactory
...
class TestLogin(APITestCase):
    ...
    def login(self, post_data=None, email=None, password=None):
        ...
        response = self.client.post(
            self.login_path, data=post_data, format='json')
        return response

    def test_successful_login_status(self):
        response = self.login(
            email=self.verified_user.email, password='test1234')
    ...
    def test_unverified_user_login_status(self):
        response = self.login(
            email=self.unverified_user.email, password='test1234')
    ...
    def test_wrong_password_login_status(self):
        response = self.login(
            email=self.verified_user.email, password='wrongpw')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestLogout(APITestCase):
    ...
    def test_proper_login_post_logout_status(self):
        self.client.login(
            self.login_path, email=self.verified_user.email,
            password='test1234')
        response = self.client.post(self.logout_path)
        assert response.status_code == status.HTTP_200_OK
...
class TestPasswordChange(APITestCase):
    ...
    def change_password(self, post_data=None, new_password1=None,
                        new_password2=None):
        if new_password1 or new_password1:
            post_data = {"new_password1": new_password1,
                         "new_password2": new_password2}
        response = self.client.post(
            self.password_change_path, data=post_data, format='json')
        return response

    def test_change_password_status(self):
        response = self.change_password(
            new_password1='new56789', new_password2='new56789')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_new_password_login(self):
        self.change_password(new_password1='new56789',
                             new_password2='new56789')
        self.client.logout()
    ...
    def test_short_password_change_error(self):
        response = self.change_password(
            new_password1='short', new_password2='short')
        assert response.data['new_password2'] == [
    ...
    def test_empty_password_error(self):
        post_data = {"new_password1": "", "new_password2": ""}
        response = self.change_password(post_data)
        assert response.data['new_password1'] == [
    ...
    def test_old_password_change_status(self):
        response = self.change_password(
            new_password1='test1234', new_password2='test1234')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_different_passwords_status(self):
        response = self.change_password(
            new_password1='new56789', new_password2='diff7654')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestPasswordResetInitiate(APITestCase):
    ...
    def test_password_reset_email_sent(self):
        self.client.post(self.password_reset_path,
                         data={"email": self.verified_user.email})
        assert len(mail.outbox) == 1

    def test_password_reset_email_subject(self):
        self.client.post(self.password_reset_path, data={
                         "email": self.verified_user.email})
    ...
    def test_empty_email_password_reset_status(self):
        response = self.client.post(
            self.password_reset_path, data={"email": ""})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_none_email_password_reset_status(self):
        response = self.client.post(
            self.password_reset_path, data={"email": None})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
