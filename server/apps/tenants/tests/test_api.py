...
from server.apps.accounts.tests.factories import (
    UserFactory, UserTenantRelationshipFactory)
...
class TenantSignupTests(TestCase):
    ...
    def setUp(self):
        ...
        already_existing_site = Site.objects.get(
            domain=self.tenant_root_domain)
        already_existing_subdomain = (
            f"{self.already_registered_company_domain}"
            f".{already_existing_site.domain}")

        Site.objects.get_or_create(
            name="First", domain=already_existing_subdomain)

        Tenant.objects.create(name=self.already_registered_company_name,
                              site=already_existing_site)
        User.objects.create(email=self.already_registered_user_email,
                            first_name="Was", last_name="First")

        MailTemplate.objects.create(
            name="email_confirmation_signup",
            html_template=("<a href='{{ activation_url }}'>"
    ...
    def sign_up(self, post_data):
        ...
        self.assertContains(
            response, "Verification e-mail sent", status_code=201)
        ...
        site = Site.objects.get(name=correct_subdomain,
                                domain=correct_subdomain)
    ...
    def sign_up_error(self, post_data, expected_error,
        ...
        self.assertEqual(User.objects.filter(
            email="awesome-ceo@example.com").count(), 0)
        self.assertEqual(Tenant.objects.filter(
            name="Awesome customer").count(), 0)
        self.assertEqual(Site.objects.filter(
            domain="awesome.{}".format(self.tenant_root_domain)).count(), 0)
    ...
    def test_already_existing_email(self):
        ...
        self.sign_up_error(
            post_data, "A user is already registered with this e-mail address")
    ...
    def test_already_existing_domain(self):
        ...
        self.sign_up_error(
            post_data, "There is already an domain with that name")
    ...
    def test_password_too_close_to_email(self):
        ...
        self.sign_up_error(
            post_data, "The password is too similar to the email address")
...
class UserSignupTests(TestCase):
    """Test the signup process of accounts."""

    def setUp(self):
        # Create valid tenant
        tenant_admin = User.objects.create_user(
            email="admin@example.com", first_name="Achim", last_name="Admin")
        self.tenant = Tenant.objects.create_tenant(
            tenant_admin, "tenant", "tenant")
        self.tenant.is_active = True
        self.tenant.save()

        self.signup_url = reverse('user_rest_register', kwargs={
    ...
    def user_signup(self, post_data, expected_status_code=201,
    ...
    def test_deactivated_tenant(self):
        ...
        self.user_signup(
            ...
            expected_error_message=(
                "Tenant is deactivated, no registration is possible")
        )
    ...
    def test_missing_password1(self):
        ...
        self.user_signup(post_data, expected_status_code=400,
...
class TestInviteCreate(APITestCase):
    ...
    def setUp(self):
        ...
        self.invite_path = reverse('rest_invite', kwargs={
                                   'tenant_name': self.tenant.name})

        MailTemplate.objects.create(
            name="invite", html_template="<p>Invite</p>")
...
class TestInviteRetrieve(APITestCase):
    ...
    def setUp(self):
        ...
        self.retrieve_path = reverse(
            ...
            kwargs={
                'tenant_name': self.invite.tenant.name,
                'pk': self.invite.pk}
        )
        ...
        self.no_name_retrieve_path = reverse(
            'rest_invite_retrieve',
            kwargs={'tenant_name': no_name_invite.tenant.name,
                    'pk': no_name_invite.pk}
        )
    ...
    def test_not_active(self):
        expired_creation_date = timezone.now(
        ) - timedelta(days=settings.TENANT_INVITE_EXPIRATION_IN_DAYS, hours=1)
    ...
    def test_existing_user(self):
        ...
        existing_user_retrieve_path = reverse(
            'rest_invite_retrieve',
            kwargs={'tenant_name': existing_user_invite.tenant.name,
                    'pk': existing_user_invite.pk}
        )
        response = self.client.get(existing_user_retrieve_path)
        assert response.data[
            'detail'
        ] == (f'Your account is now successfully '
              'connected to {existing_user_invite.tenant.name}.')
...
class TestInviteActivation(APITestCase):
    ...
    def setUp(self):
        ...
        self.response = self.client.patch(
            self.activation_url, data=self.post_data, format='json')
    ...
    def test_existing_user(self):
        existing_user = UserFactory()
        existing_user_invite = InviteFactory(email=existing_user.email)
        existing_user_activation_url = (
            existing_user_invite.get_activation_url())
        response = self.client.patch(
            existing_user_activation_url, data=self.post_data, format='json')
        assert response.data[
            'error'] == (
            'You are already a user, please use the link from the email.')
    ...
    def test_inactive_invite(self):
        ...
        response = self.client.patch(
            old_invite_activation_url, data=self.post_data, format='json')
        assert response.data[
            'error'] == 'The invitation was not used while it was active.'
