...
from server.apps.tenants.tests.factories import InviteFactory, TenantFactory
...
class TestTenantsURLs(TestCase):
    ...
    def test_tenants_registration_rest_register_resolve(self):
        assert resolve(
            '/api/sign-up/tenant/').view_name == 'tenant_rest_register'
    ...
    def test_tenants_registration_rest_invite_retrieve_reverse(self):
        ...
        assert reverse(
            ...
            kwargs={
                'tenant_name': self.tenant.name, 'pk': invite.pk}) ==\
    ...
    def test_tenants_registration_rest_invite_activation_reverse(self):
        ...
        assert reverse(
            ...
            kwargs={'tenant_name': self.tenant.name, 'pk': invite.pk}
        ) == (
            f'/api/sign-up/tenant/{self.tenant.name}/'
            'invite/{invite.pk}/activate/')
    ...
    def test_tenants_registration_email_verification_sent_reverse(self):
        assert reverse(
            'account_email_verification_sent') == '/api/sign-up/confirm-email/'

    def test_tenants_registration_email_verification_sent_resolve(self):
        assert resolve(
            '/api/sign-up/confirm-email/'
