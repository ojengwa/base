...
def validate_default_site(value):
    ...
    if value == tenant_domain_site_id:
        raise ValidationError(
            f'The root domain {{tenant_root_domain}} can not be used.')
...
class TenantManager(models.Manager):
    ...
    def create_tenant(self, user, name, subdomain):
        ...
        try:
            ...
            raise ValidationError(
                f'The domain {{domain}} is already used by another tenant.')
    ...
    def get_tenant_root_domain(self):
        ...
        raise ImproperlyConfigured(
            "You're using the Tenant model without having"
            " TENANT_ROOT_SITE_ID setting."
...
class Tenant(UUIDMixin):
    ...
    name = models.CharField(max_length=100, help_text=_(
        u"Name of the team (the agency or company)"), unique=True)

    is_active = models.BooleanField(
        _('active'),
        default=False,
        help_text=_(
            'Designates whether this tenant should be treated as active. '),
    )

    date_joined = models.DateTimeField(_('date joined'), help_text=_(
        "When did the tenant join?"), default=timezone.now)
    ...
    def add_user(self, user):
        from server.apps.users.models import UserTenantRelationship
...
class Domain(models.Model):
    """Every tenant can have several
    extra-domains leading to his site subdomain."""
...
class TenantMixin(models.Model):
    ...
    tenant = models.ForeignKey(
        Tenant, null=False, blank=False, on_delete=models.CASCADE)
...
class Invite(TenantMixin, UUIDMixin):
    ...
    first_clicked = models.DateTimeField(
        _("first clicked"),
        help_text=_(
            "When was the first time the invitee clicked on the invite?"),
        null=True,
        blank=True
    )
    ...
    def get_activation_url(self):
        path = reverse('rest_invite_activation', kwargs={
                       'tenant_name': self.tenant.name, 'pk': self.pk})
        return path
    ...
    @property
    def is_active(self):
        return timezone.now() - self.time_created < timedelta(
            days=settings.TENANT_INVITE_EXPIRATION_IN_DAYS)
