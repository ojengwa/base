...
class TenantMiddleware(object):
    ...
    - subdomains of the TENANT_ROOT_SITE_ID
        (a.example.com, bar.example.com, foo.example.com) - each tenant has one
    ...
    def __call__(self, request):
        ...
        # Account for PORT address
        host_name = request.get_host().split(':')[0]

        site_id = None
        request.tenant = None

        # Case 1: An existing site (either belonging to the marketing page
        # itself or a tenant)
        try:
        ...
        # Case 2 - it is an external domain belonging to a tenant, not
        # registered within the Site model
        try:
        ...
        logger.debug(
            "TenantMiddleware: Host: {}, Site_ID: {}, Tenant: {}".format(
                host_name, site_id, request.tenant))

        if site_id:
            settings.SITE_ID = site_id
        else:
            # Case 3 - somebody is accessing a tenant subdomain page, but it
            # does not exist, raise 404
            ...
            if (len(domain_parts) == 2 and
                domain_parts[1] == tenant_root_domain) or \
                    (len(domain_parts) == 1 and
                     domain_parts[0] == tenant_root_domain):
                raise Http404
