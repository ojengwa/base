...
from .views import (
    GetAllowedVerifyEmailView, InviteActivationView, InviteCreateView,
    InviteRetrieveView, TenantRegisterView, TenantUserRegisterView)

urlpatterns = [
    url(r'^tenant/$', TenantRegisterView.as_view(),
        name='tenant_rest_register'),
    url(r'^tenant/(?P<tenant_name>[-:\w]+)/user/$',
        TenantUserRegisterView.as_view(),
        name='user_rest_register'),

    url(r'^tenant/(?P<tenant_name>[-:\w]+)/invite/$',
    ...
    url((r'^tenant/(?P<tenant_name>[-:\w]+)/'
         r'invite/(?P<pk>[0-9a-f-]+)/activate/$'
    ...
    url(r'^confirm-email/(?P<key>[-:\w]+)/$',
        GetAllowedVerifyEmailView.as_view(), name='account_confirm_email'),
]
