...
from server.apps.accounts.serializers import CreateUserSerializer
from ..models import Invite, Tenant
from ..serializers import (
    InviteActivationCreateUserSerializer,
    InviteCreateSerializer, InviteRetrieveSerializer)


class TenantRegisterView(RegisterView):
    """Sign up for a tenant
...
class TenantUserRegisterView(RegisterView):
    ...
    When the tenant is registered and activated,
    users can register for the tenants space.
    """
...
class InviteRetrieveView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if invite.is_active and existing_user:
            ...
            invite.save()
            return Response(
                {'detail': _(f"Your account is now successfully"
                             f" connected to {invite.tenant.name}.")})
...
class InviteActivationView(UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if invite.user:
            ...
            return Response({'error': _(
                "The invitation was not used while it was active.")},
