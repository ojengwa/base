...
from .factories import CommentUserFactory
...
class TestPostCorrectComment(APITestCase):
    ...
    def setUp(self):
        ...
        self.endpoint = reverse('users:user-comments',
    ...
    def test_id_of_commented_object(self):
        assert self.response.data['object_id'] == str(self.commented_user.pk)

    def test_type_of_commented_object(self):
...
class TestPostUnauthorizedComment(APITestCase):
    ...
    def test_post_unauthorized_status(self):
...
class TestGetComments(APITestCase):
    ...
    def test_last_result_should_contain_content_of_last_comment(self):
        assert self.response.data[
            'results'][-1]['content'] == self.comment3.content
    ...
    def test_result_should_contain_right_object_id(self):
        assert self.response.data['results'][0][
            'object_id'] == str(self.commented_user.pk)

    def test_result_should_contain_right_content_type(self):
        assert self.response.data['results'][0][
            'content_type'] == self.comment1.content_type.pk
