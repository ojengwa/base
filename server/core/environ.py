...
class Env(object):
    ...
    _CACHE_BASE_OPTIONS = ['TIMEOUT', 'KEY_PREFIX',
                           'VERSION', 'KEY_FUNCTION', 'BINARY']
    ...
    def list(self, var, cast=None, default=NOTSET):
        ...
        :rtype: list
        """
        return self.get_value(var, cast=list if not cast else [cast],
                              default=default)
    ...
    def tuple(self, var, cast=None, default=NOTSET):
        ...
        :rtype: tuple
        """
        return self.get_value(var, cast=tuple if not cast else (cast,),
                              default=default)
    ...
    def url(self, var, default=NOTSET):
        ...
        """
        return self.get_value(var, cast=urllib.parse.urlparse,
                              default=default, parse_default=True)
    ...
    def db_url(self, var=DEFAULT_DATABASE_ENV, default=NOTSET, engine=None):
        ...
        :rtype: dict
        """
        return self.db_url_config(self.get_value(var, default=default),
                                  engine=engine)
    db = db_url
    ...
    def cache_url(self, var=DEFAULT_CACHE_ENV, default=NOTSET, backend=None):
        ...
        :rtype: dict
        """
        return self.cache_url_config(self.url(var, default=default),
                                     backend=backend)
    cache = cache_url
    ...
    def email_url(self, var=DEFAULT_EMAIL_ENV, default=NOTSET, backend=None):
        ...
        :rtype: dict
        """
        return self.email_url_config(self.url(var, default=default),
                                     backend=backend)
    email = email_url
    ...
    def search_url(self, var=DEFAULT_SEARCH_ENV, default=NOTSET, engine=None):
        ...
        :rtype: dict
        """
        return self.search_url_config(self.url(var, default=default),
                                      engine=engine)
    ...
    def parse_value(cls, value, cast):
        ...
        if cast is None:
            ...
            value = dict(map(
                lambda kv: (
                    key_cast(kv[0]),
                    cls.parse_value(kv[1], value_cast_by_key.get(
                        kv[0], value_cast))
                ),
    ...
    def db_url_config(cls, url, engine=None):
        ...
        if url.scheme == 'sqlite' and path == '':
            path = ':memory:'
        if url.scheme == 'ldap':
            path = '{scheme}://{hostname}'.format(scheme=url.scheme,
    ...
    def cache_url_config(cls, url, backend=None):
        ...
        :return:
        """
        url = urllib.parse.urlparse(url) if not isinstance(
        ...
        if url.path and url.scheme in ['memcache', 'pymemcache']:
            ...
            if url.hostname:
                ...
                scheme = 'unix'
            locs = [
                scheme + '://' + l + url.path for l in url.netloc.split(',')]

            config['LOCATION'] = locs[0] if len(locs) == 1 else locs
    ...
    def search_url_config(cls, url, engine=None):
        ...
        if url.query:
            ...
            if 'EXCLUDED_INDEXES' in params.keys():
                config['EXCLUDED_INDEXES'] = params[
                    'EXCLUDED_INDEXES'][0].split(',')
        ...
        if url.scheme == 'solr':
            config['URL'] = urllib.parse.urlunparse(
                ('http',) + url[1:2] + (path,) + ('', '', ''))
        ...
        if url.scheme == 'whoosh':
            ...
            if 'POST_LIMIT' in params.keys():
                config['POST_LIMIT'] = cls.parse_value(params['POST_LIMIT'][0],
                                                       int)
    ...
    def read_env(cls, env_file=None, **overrides):
        ...
        if env_file is None:
            ...
            env_file = os.path.join(os.path.dirname(
                frame.f_back.f_code.co_filename), '.env')
...
class Path(object):
    ...
    def __add__(self, other):
        return Path(self.__root__, other if not isinstance(
            other, Path) else other.__root__)
