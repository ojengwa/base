...
from server.apps.accounts.serializers import (
...
class UserRegistrationSerializerTest(CustomTestCase, APITestCase):
    INVALID_DATA_DICT = [
        {'data': {'email': 'test1@mailinator.com',
                  ...
                  'password': 'test'},
         'error': (
            'email', ['Please use a different email address provider.']),
         'label': 'Invalid email.',
         'method': 'POST',
    ...
    def test_fields(self):
        ...
        self.assert_fields_required(
            False, serializer, self.not_required_fields)
        self.assertEqual(len(serializer.fields), len(
            self.required_fields) + len(self.not_required_fields))
